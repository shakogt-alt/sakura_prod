...
## 6. Счётчик посещений — ветка visits-data

Счётчик посещений (вкладка «Посещения») хранит данные не в `main`, а в отдельной ветке `visits-data` того же репозитория. Сделано так специально: каждое посещение сайта — это отдельный коммит, и если бы он попадал в `main`, любой заход на сайт мог столкнуться с вашим `git push` («non-fast-forward», приходилось бы делать `git pull` перед каждым пушем). В `visits-data` эти коммиты никому не мешают — вы её никогда не пуллите и не пушите руками.

Ветка создаётся один раз (уже сделано при добавлении функции) и должна быть на GitHub — если её там нет, счётчик перестанет работать (посещения не будут считаться, но сайт при этом не сломается). Проверить: `https://github.com/shakogt-alt/sakura_prod/branches` — `visits-data` должна быть в списке.
